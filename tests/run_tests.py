...
def test_active_tenant_logic():
    """Test 3: Active Tenant Logic - Active tenants cannot exceed room capacity"""
    try:
        ...
        for room in Room.objects.all():
            ...
            if active_tenants > room.capacity:
                rooms_with_multiple_active.append(f"Room {room.number}: {active_tenants} active tenants (capacity {room.capacity})")
        
        if rooms_with_multiple_active:
            results.add_fail("Active Tenant Logic", f"Over-capacity active tenants: {', '.join(rooms_with_multiple_active)}")
        else:
            results.add_pass("Active Tenant Logic", "Active tenant counts are within room capacities")
...
def test_room_availability():
    ...
    try:
        ...
        for room in Room.objects.all():
            ...
            # Room should not exceed capacity
            if active_tenants > room.capacity:
                warnings.append(f"Room {room.number}: {active_tenants} active tenants (capacity {room.capacity})")
        
        if warnings:
            results.add_warning("Room Availability", f"Issues: {'; '.join(warnings[:5])}")
        else:
            results.add_pass("Room Availability", "All rooms have valid tenant assignments within capacity")
