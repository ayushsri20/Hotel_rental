...
def test_tenant_assignment_validation():
    ...
    # Try to add third tenant (should fail at model validation)
    try:
        guest3 = Guest(
            first_name="Test",
            last_name="User3",
        ...
        msg = e.message_dict.get('room', [str(e)])[0]
        print(f"✅ Third tenant blocked: {msg}")
...
def test_room_filtering():
    ...
    assert available[0].number == "T-PARTIAL", "Partial room should be first"
    ...
    # Clean up
    Guest.objects.filter(room__number__startswith="T-").delete()
    Room.objects.filter(number__startswith="T-").delete()
